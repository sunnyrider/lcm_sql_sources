SELECT * FROM BSI_COMPANY cmp WHERE COMPANY_NO = '6175913';
...
SELECT DISTINCT(UCT.UC_UID) AS D_ID  FROM BSI_X_EXT_JOIN_JOIN ejj INNER JOIN BSI_X_EXT_JOIN ej
	INNER JOIN BSI_UC uc
		INNER JOIN BSI_UC_TEXT uct
			ON UCT.UC_UID = UC.UC_UID
		ON UC.UC_UID = EJ.SEGMENTATION_UID
	ON EJ.EXT_JOIN_NR = EJJ.EXT_JOIN_NR
WHERE UCT.LANGUAGE_UID = 246
AND UC.IS_ACTIVE = 1
;

SELECT DISTINCT(CATEGORY_UID) FROM BSI_EMPLOYEE;

SELECT DISTINCT(UCT.UC_UID) AS D_ID  FROM BSI_COMPANY cmp
INNER JOIN BSI_UC uc
	INNER JOIN BSI_UC_TEXT uct
		ON UCT.UC_UID = UC.UC_UID
	ON UC.UC_UID = CMP.X_ADVISORY_STATUS_UID
WHERE UCT.LANGUAGE_UID = 246
AND UC.IS_ACTIVE = 1
;

SELECT UC_UID, TEXT FROM BSI_UC_TEXT
WHERE UC_UID IN (
117411,
117413
)
AND LANGUAGE_UID = 246
ORDER BY TEXT;
...
SELECT * FROM SYS.ALL_TAB_COLUMNS WHERE LOWER(COLUMN_NAME) LIKE '%categor%'
AND TABLE_NAME LIKE 'BSI%';