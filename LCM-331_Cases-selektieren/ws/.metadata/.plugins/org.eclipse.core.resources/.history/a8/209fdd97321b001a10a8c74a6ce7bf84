WITH preselect AS
(
	SELECT /*+ ordered use_hash(CSE CMC) full(CSE) full(CMC) */
		CSE.CASE_NO,
		CSE.EVT_START,
		CSE.EVT_END,
		CSE.EMAIL_ADDRESS,
	...
	INNER JOIN BSICRM.BSI_COMMUNICATION cmc ON CMC.COMPANY_NR = CSE.X_COMPANY_NR
...
--	ORDER BY CSE.CASE_NO DESC, CMC.COMMUNICATION_NR DESC
)
SELECT
...
--	case_join.max_case_no case_number,
	...
	(
		SELECT /*+ ordered use_hash(bdoc) full(bdoc) */
			SUBSTR(LISTAGG(dbms_lob.substr(BDOC.X_LOCATION_LOB, 2000, 1), '|') within GROUP(ORDER BY BDOC.X_LOCATION_LOB), 0, 3000)
		FROM BSI_DOCUMENT bdoc
		WHERE BDOC.ITEM_KEY0_NR = CMC.COMMUNICATION_NR
		AND dbms_lob.substr(BDOC.X_LOCATION_LOB, 200, 1) LIKE '%NX01%'
	) LCM_Document_link
FROM preselect PSEL
	INNER JOIN BSICRM.BSI_COMMUNICATION cmc
	ON CMC.COMMUNICATION_NR = PSEL.COMMUNICATION_NR 
...
--		ICSE.PHONE_NO
--		FROM BSI_CASE icse
--		GROUP BY ICSE.CASE_FRAME_NR, ICSE.PHONE_NO
--	) case_join ON case_join.CASE_FRAME_NR = psel.CASE_FRAME_NR
--	AND case_join.PHONE_NO = psel.PHONE_NO
